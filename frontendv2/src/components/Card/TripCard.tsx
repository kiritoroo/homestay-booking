...
import {
  isShowCancelPolicyModalAtom,
  isShowTripDetailModalAtom,
  selectedBookingIdAtom,
...
export const Tripcard = (props: Props) => {
  ...
  const [isShowCancelPolicyModal, setIsShowCancelPolicyModal] = useRecoilState(
    isShowCancelPolicyModalAtom
  );
  const [isShowTripDetailModal, setIsShowTripDetailModal] = useRecoilState(
    isShowTripDetailModalAtom
  );
  const [selectedBookingId, setSelectedBokkingId] = useRecoilState(
    selectedBookingIdAtom
  );
  console.log(booking);
  ...
  const handleCancleBooking = useCallback(() => {
    ...
    bookingActions.abort(username, homestayId, bookingId).then((res) => {});
  }, [booking]);
  ...
  return (
    <S.StyledContainer>
      <S.StyledImage
        onClick={handleViewDetailMouseClick}
        src={booking.homestay_booking.main_image}
      />
      <S.StyledBooInfoWrapper onClick={handleViewDetailMouseClick}>
        <S.StyledBookInfoTitle onClick={handleViewDetailMouseClick}>
          Mã đặt phòng/đặt chỗ: &nbsp;{" "}
        </S.StyledBookInfoTitle>
        <S.StyledBookInfoValue>
      ...
      <S.StyledBooInfoWrapper onClick={handleViewDetailMouseClick}>
        ...
        {booking.payment.status == "unpaid" ? (
          <S.StyledBookInfoValue style={{ color: "#7D97B8" }}>
            {" "}
            Chưa thanh toán{" "}
          </S.StyledBookInfoValue>
        ) : booking.payment.status == "invalidated" ? (
          <S.StyledBookInfoValue style={{ color: "#C13515" }}>
            {" "}
            Hủy thanh toán{" "}
          </S.StyledBookInfoValue>
        ) : (
          <S.StyledBookInfoValue style={{ color: "#3DC373" }}>
            {" "}
            Đã thanh toán{" "}
          </S.StyledBookInfoValue>
        )}
      ...
      <S.StyledBooInfoWrapper onClick={handleViewDetailMouseClick}>
        ...
        {booking.booking.status == "validated" ? (
          ...
          <S.StyledBookInfoValue style={{ color: "#05D619" }}>
            {" "}
            Đã xác nhận{" "}
          </S.StyledBookInfoValue>
        )}
      ...
      <S.StyledDateInfoWrapper onClick={handleViewDetailMouseClick}>
        <S.StyledDateCheckInWrapper>
          ...
          <S.StyledDateInfoValue>
            {booking.booking.status != "cancel"
              ? formatUtils.formatDateString(
                  new Date(booking.booking.checkin_date)
                )
              : "___"}
          </S.StyledDateInfoValue>
        </S.StyledDateCheckInWrapper>
        ...
        <S.StyledDateCheckOutWrapper>
          ...
          <S.StyledDateInfoValue>
            {booking.booking.status != "cancel"
              ? formatUtils.formatDateString(
                  new Date(booking.booking.checkout_date)
                )
              : "___"}
          </S.StyledDateInfoValue>
        </S.StyledDateCheckOutWrapper>
      ...
      {booking.booking.status == "validated" ? (
        <S.StyledButtonAbort onClick={handleCancleButtonMouseClick}>
          Hủy chuyến đi
        </S.StyledButtonAbort>
      ) : (
        <S.StyledButtonReBook onClick={handleReBooking}>
          Đặt lại chuyến đi
        </S.StyledButtonReBook>
      )}

      {isShowCancelPolicyModal &&
        selectedBookingId == booking.booking.booking_id && (
          <CancelPolicyModal onCancel={handleCancleBooking} booking={booking} />
        )}
      {isShowTripDetailModal &&
        selectedBookingId == booking.booking.booking_id && (
          <TripDetailModal booking={booking} />
        )}
    </S.StyledContainer>
  );
};
